...
class App:
    ...
    def convert_to_invoice(self):
        ...
        ttk.Label(payment_summary, text="Invoice Total:", font=("Segoe UI",10,"bold")).pack(side="left")
        ttk.Label(payment_summary, textvariable=total_var, font=("Segoe UI",10,"bold")).pack(side="left", padx=(8,20))
        ...
        def save_invoice_pdf():
            ...
            y -= 3*mm
            txt(f"CUSTOMER   : {customer.get().strip()}",True,11)
            ...
            y -= 2*mm; txt("PAYMENT BREAKDOWN",True,9)
            for pm, pa in payment_lines(): txt(f"  {pm:<22} {money(pa):>15}",False,8.5)
            txt("PAYMENT TOTAL : " + money(sum(pa for _, pa in payment_lines())),True,9)
